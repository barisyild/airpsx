...
// WIP
class FileSystemDownloadHandler extends AbstractHttpFileStreamHandler {
    ...
    public override function execute(request:Request):AbstractResponse {
        ...
        if(isArchive)
        {
            ...
            var response = ResponseBuilder.asStatic();
            response.contentLength = tarFileSize;
            response.header("Content-Disposition", 'attachment; filename=\"${fileName}.tar\"');
